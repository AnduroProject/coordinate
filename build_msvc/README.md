...
To build Bitcoin Core without Qt, unload or disable the `coordinate-qt`, `libcoordinate_qt` and `test_coordinate-qt` projects.
...
To check if `coordinated` has `RandomizedBaseAddress` enabled or disabled run

```
.\dumpbin.exe /headers src/coordinated.exe
```
