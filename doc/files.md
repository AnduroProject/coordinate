...
`./`               | `bitcoin.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `coordinated` or `coordinate-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `coordinated.pid`        | Stores the process ID (PID) of `coordinated` or `coordinate-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `coordinated` or `coordinate-qt`; can be specified by `-debuglogfile` option
...
`coordinate-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).
