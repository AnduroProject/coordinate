...
CJDNS peers than Tor or IP ones. You can use `coordinate-cli -addrinfo` to see the
...
To see which CJDNS peers your node is connected to, use `coordinate-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `coordinate-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `coordinate-cli help getnodeaddresses` for details.
