...
def main(coordinated_path):
    coordinated_with_usdts = USDT(path=str(coordinated_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    coordinated_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    coordinated_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    coordinated_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    coordinated_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[coordinated_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/coordinated")
        exit(1)
