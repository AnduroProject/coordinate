# bash programmable completion for coordinate-cli(1)
# Copyright (c) 2012-2022 The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $coordinate-cli for RPC
_bitcoin_rpc() {
    ...
    $coordinate_cli "${rpcargs[@]}" "$@"
}

_coordinate_cli() {
    local cur prev words=() cword
    local coordinate_cli

    # save and use original argument to invoke coordinate-cli for -help, help and RPC
    # as coordinate-cli might not be in $PATH
    coordinate_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($coordinate_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi
...
complete -F _coordinate_cli coordinate-cli
