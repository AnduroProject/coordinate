...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing coordinate-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='coordinate-cli', help='coordinate-cli command to use')
...
parser.add_argument('coordinate_cli_args', nargs='*', help='Arguments to pass on to coordinate-cli (default: -signet)')

args = parser.parse_args()

if args.coordinate_cli_args == []:
    args.coordinate_cli_args = ['-signet']


def coordinate_cli(rpc_command_and_params):
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = coordinate_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = coordinate_cli(['getnewaddress', 'faucet', 'bech32'])
