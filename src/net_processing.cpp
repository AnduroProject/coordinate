...
public:
    PeerManagerImpl(CConnman& connman, AddrMan& addrman,
                    ...
                    CTxMemPool& pool, CTxMemPool& preconfpool, Options opts);
...
std::unique_ptr<PeerManager> PeerManager::make(CConnman& connman, AddrMan& addrman,
                                               ...
                                               CTxMemPool& pool, CTxMemPool& preconfpool, Options opts)
{
    return std::make_unique<PeerManagerImpl>(connman, addrman, banman, chainman, pool, preconfpool, opts);
}
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::HEADERS)
    {
        ...
        std::vector<CBlockHeader> headersmain;
        vRecv >> headers;

        for (size_t i = 0; i < headers.size(); i++)
        {
            headersmain.push_back(headers[i].GetBlockHeader());
        }
        ...
        ProcessHeadersMessage(pfrom, *peer, std::move(headersmain), /*via_compact_block=*/false);
