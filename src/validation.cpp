...
bool CheckProofOfWork(const CBlockHeader& block, const Consensus::Params& params)
{
    ...
    if (!block.auxpow)
    {
        // mainnet - 31902208f1dc57e1f009d20bfab5116303487ecd0840e27d103af98a9f1a4298
        // testnet - d056cca6fa864bc8099fbdf587e251ff247e33cdfbeb1ea481e607e7f9722f9c
        // regtest - 11de75a48e2193f7e88104eb64a078eaff54b7d6ec2ffbd97cb15edc128d498d
        if(block.GetHash().ToString().compare("31902208f1dc57e1f009d20bfab5116303487ecd0840e27d103af98a9f1a4298") == 0 || block.GetHash().ToString().compare("d056cca6fa864bc8099fbdf587e251ff247e33cdfbeb1ea481e607e7f9722f9c") == 0  || block.GetHash().ToString().compare("11de75a48e2193f7e88104eb64a078eaff54b7d6ec2ffbd97cb15edc128d498d") == 0) {
            return true;
        } else {
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight, CAmount& amountAssetInOut, int& nControlNOut, uint32_t& nAssetIDOut, uint32_t nNewAssetIDIn, CAmount& preconfCurrentFee)
{
    ...
    // add outputs
    AddCoins(inputs, tx, nHeight, preconfCurrentFee, nAssetIDOut, amountAssetInOut, nControlNOut, nNewAssetIDIn, preconfCurrentFee);
}
    ...
    for (const SignedBlock& finalizedSignedBlock : block.preconfBlock) {
        if (!checkSignedBlock(finalizedSignedBlock, m_chainman)) {
            ...
            for (unsigned int i = 0; i < finalizedSignedBlock.vtx.size(); i++) {
                ...
                int nControlN = -1;
                uint32_t nAssetID = 0;
                CAmount preconfCurrentFee = CAmount(finalizedSignedBlock.currentFee);
                UpdateCoins(tx, view, undoDummy, pindex->nHeight, amountAssetIn, nControlN, nAssetID, 0, preconfCurrentFee);
            }
...
CCoinsViewCache& Chainstate::UpdatedCoinsTip(CCoinsViewCache& view, int blockHeight) {
    for (SignedBlock& finalizedSignedBlock : getFinalizedSignedBlocks()) {
        for (unsigned int i = 0; i < finalizedSignedBlock.vtx.size(); i++) {
            ...
            uint32_t nAssetID = 0;
            CAmount preconfCurrentFee = CAmount(finalizedSignedBlock.currentFee);
            UpdateCoins(tx, view, undoDummy, blockHeight, amountAssetIn, nControlN, nAssetID, 0, preconfCurrentFee);
        }
    }
...
bool Chainstate::ConnectSignedBlock(const SignedBlock& block) {
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        int nControlN = -1;
        uint32_t nAssetID = 0;
        CAmount preconfCurrentFee = CAmount(block.currentFee);
        UpdateCoins(tx, view, i == 0 ? undoDummy : blockundo.vtxundo.back(), m_chainman.ActiveHeight(), amountAssetIn, nControlN, nAssetID, 0, preconfCurrentFee);
    }
...
bool Chainstate::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs)
{
    ...
    for (size_t x = 0; x < block.vtx.size(); x++) {
        ...
        AddCoins(inputs, *tx, pindex->nHeight, preconfCurrentFee, amountAssetIn, nControlN, true);
    }
