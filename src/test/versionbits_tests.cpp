...
static void check_computeblockversion(VersionBitsCache& versionbitscache, const Consensus::Params& params, Consensus::DeploymentPos dep, ChainType chain_type)
{
    ...
    if (chain_type == ChainType::MAIN) {
        BOOST_REQUIRE_EQUAL(min_activation_height % params.nMinerConfirmationWindow, 12U);
    } else {
    ...
    if (chain_type == ChainType::MAIN) {
        BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit), 4);
    } else {
        BOOST_CHECK_EQUAL(versionbitscache.ComputeBlockVersion(lastBlock, params) & (1 << bit), 0);
    }
}
...
BOOST_AUTO_TEST_CASE(versionbits_computeblockversion)
{
    ...
    for (const auto& chain_type : {ChainType::MAIN, ChainType::TESTNET, ChainType::SIGNET, ChainType::REGTEST}) {
        ...
        for (int i = 0; i < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; ++i) {
            ...
            check_computeblockversion(vbcache, chainParams->GetConsensus(), dep, chain_type);
        }
    }

    {
        ...
        check_computeblockversion(vbcache, chainParams->GetConsensus(), Consensus::DEPLOYMENT_TESTDUMMY, ChainType::REGTEST);
    }
