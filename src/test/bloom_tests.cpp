...
BOOST_FIXTURE_TEST_CASE(merkle_block_1, TestChain100Setup)
{
    ...
    BOOST_CHECK(merkleBlock.vMatchedTxn[0].second == hashv);
    ...
    BOOST_CHECK(merkleBlock.vMatchedTxn[1].second == hashv2);
...
BOOST_FIXTURE_TEST_CASE(merkle_block_2, TestChain100Setup)
{
    ...
    coinbaseTx.vout.resize(1);
    coinbaseTx.vin[0].scriptSig = CScript() << OP_11 << OP_EQUAL;
    coinbaseTx.vout[0].nValue = 0.1 * CENT;
    const uint256& hashv = coinbaseTx.GetHash();
    filter.insert(hashv);
    block.vtx.push_back(MakeTransactionRef (coinbaseTx));
    ...
    coinbaseTx2.vout.resize(1);
    coinbaseTx2.vin[0].scriptSig = CScript() << OP_11 << OP_EQUAL;
    coinbaseTx2.vout[0].nValue = 0.2 * CENT;
    const uint256& hashv2 = coinbaseTx2.GetHash();
    filter.insert(hashv2);
    block.vtx.push_back(MakeTransactionRef (coinbaseTx2));

   CMutableTransaction coinbaseTx3 = CMutableTransaction();
    coinbaseTx3.nVersion = 1;
    coinbaseTx3.vin.resize(1);
    coinbaseTx3.vout.resize(1);
    coinbaseTx3.vin[0].scriptSig = CScript() << OP_11 << OP_EQUAL;
    coinbaseTx3.vout[0].nValue = 0.3 * CENT;
    const uint256& hashv3 = coinbaseTx3.GetHash();
    filter.insert(hashv3);
    block.vtx.push_back(MakeTransactionRef (coinbaseTx3));

    CMutableTransaction coinbaseTx4 = CMutableTransaction();
    coinbaseTx4.nVersion = 1;
    coinbaseTx4.vin.resize(1);
    coinbaseTx4.vout.resize(1);
    coinbaseTx4.vin[0].scriptSig = CScript() << OP_11 << OP_EQUAL;
    coinbaseTx4.vout[0].nValue = 0.4 * CENT;
    const uint256& hashv4 = coinbaseTx4.GetHash();
    filter.insert(hashv4);
    block.vtx.push_back(MakeTransactionRef (coinbaseTx4));
    ...
    BOOST_CHECK(merkleBlock.vMatchedTxn[2].second == hashv3);
    BOOST_CHECK(merkleBlock.vMatchedTxn[2].first == 2);

    BOOST_CHECK(merkleBlock.vMatchedTxn[3].second == hashv4);
...
BOOST_FIXTURE_TEST_CASE(merkle_block_2_with_update_none, TestChain100Setup)
{
    // Random real block (000000005a4ded781e667e06ceefafb71410b511fe0d5adc3e5a27ecbec34ae6)
    // With 4 txes
    CBlock block;
...
BOOST_FIXTURE_TEST_CASE(merkle_block_3_and_serialize, TestChain100Setup)
{
    ...
    std::string strHex = HexStr(merkleStream);

    std::vector<uint8_t> expected = ParseHex("0100000079cda856b143d9db2c1caff01d1aecc8630d30625d10e8b4b8b0000000000000b50cc069d6a3e33e3ff84a5c41d9d3febe7c770fdcc96b2c3ff60abe184f196367291b4d4c86041b8fa45d6300000000000100000001dc56a2214432f2d6c41c8b745ed3e714f8d697c20ce157856e6b270d3068798f0101");
    auto result{MakeUCharSpan(merkleStream)};

    BOOST_CHECK_EQUAL_COLLECTIONS(expected.begin(), expected.end(), result.begin(), result.end());
}

BOOST_FIXTURE_TEST_CASE(merkle_block_4, TestChain100Setup)
{
    ...
    pair = merkleBlock.vMatchedTxn[1];
    BOOST_CHECK(merkleBlock.header.GetHash() == block.GetHash());
...
BOOST_FIXTURE_TEST_CASE(merkle_block_4_test_p2pubkey_only, TestChain100Setup)
{
    // Random real block (000000000000b731f2eef9e8c63173adfb07e41bd53eb0ef0a6b720d6cb6dea4)
    // With 7 txes
    CBlock block;
    CDataStream stream(ParseHex(""), SER_NETWORK, PROTOCOL_VERSION);
    stream >> block;
    ...
    coinbaseTx.vout[0].nValue = 0.1 * CENT;
    const uint256& hashv = coinbaseTx.GetHash();
    filter.insert(COutPoint(hashv,0));
    block.vtx.push_back(MakeTransactionRef(coinbaseTx));
   

    // block.vtx[0].payload = ParseHex("04eaafc2314def4ca98ac970241bcab022b9c1e1f4ea423a20f134c876f2c01ec0f0dd5b2e86e7168cefe0d81113c3807420ce13ad1357231a2252247d97a46a91");
    ...
    coinbaseTx2.vout[0].nValue = 0.2 * CENT;
    const uint256& hashv2 = coinbaseTx2.GetHash();
    filter.insert(COutPoint(hashv2,0));
    ...
    BOOST_CHECK(filter.contains(COutPoint(hashv2,0)));
    // ... but not the 4th transaction's output (its not pay-2-pubkey)
    BOOST_CHECK(!filter.contains(COutPoint(uint256S("0x02981fa052f0481dbc5868f4fc2166035a10f27a03cfd2de67326471df5bc041"), 0)));
}

BOOST_FIXTURE_TEST_CASE(merkle_block_4_test_update_none, TestChain100Setup)
{
    // Random real block (000000000000b731f2eef9e8c63173adfb07e41bd53eb0ef0a6b720d6cb6dea4)
    // With 7 txes
    CBlock block;
    ...
    coinbaseTx.vout[0].nValue = 0.1 * CENT;
    const uint256& hashv = coinbaseTx.GetHash();
    filter.insert(COutPoint(hashv,1));
    block.vtx.push_back(MakeTransactionRef(coinbaseTx));
    ...
    // We should match the generation outpoint
    BOOST_CHECK(!filter.contains(COutPoint(hashv,0)));
    // ... but not the 4th transaction's output (its not pay-2-pubkey)
    BOOST_CHECK(!filter.contains(COutPoint(uint256S("0x02981fa052f0481dbc5868f4fc2166035a10f27a03cfd2de67326471df5bc041"), 0)));
}
