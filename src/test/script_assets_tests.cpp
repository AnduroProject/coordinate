...
static std::vector<unsigned int> AllConsensusFlags()
{
    ...
    for (unsigned int i = 0; i < 128; ++i) {
        ...
        if (i & 128) flag |= SCRIPT_VERIFY_P2MR;
        ...
        // SCRIPT_VERIFY_P2MR requires SCRIPT_VERIFY_WITNESS
        if (flag & SCRIPT_VERIFY_P2MR && !(flag & SCRIPT_VERIFY_WITNESS)) continue;
    ...
    if (test.exists("success")) {
        ...
        for (const auto flags : ALL_CONSENSUS_FLAGS) {
            ...
            if (fin || ((flags & test_flags) == flags)) {
                ...
                // For P2MR scripts, only run with P2MR flags, not Taproot flags
                if (is_p2mr_script && (flags & SCRIPT_VERIFY_TAPROOT)) {
                    continue; // Skip Taproot validation for P2MR scripts
                }
                ...
                if (!ret) {
                    ...
                    std::cout << "Is P2MR: " << (is_p2mr_script ? "true" : "false") << std::endl;
                }
                BOOST_CHECK(ret);
            }
        }
